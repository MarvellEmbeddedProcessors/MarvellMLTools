...
def get_ddos_flows(attackers, victims):
    ...
    if '/' in attackers:  # subnet
        DDOS_FLOWS['attackers'] = [str(ip) for ip in list(
            ipaddress.IPv4Network(attackers).hosts())]
    else:  # single address
        DDOS_FLOWS['attackers'] = [str(ipaddress.IPv4Address(attackers))]

    if '/' in victims:  # subnet
        DDOS_FLOWS['victims'] = [str(ip) for ip in list(
            ipaddress.IPv4Network(victims).hosts())]
    else:  # single address
...
def parse_packet(pkt):
    ...
    try:
        ...
        protocols = vector_proto.transform(
            [pkt.frame_info.protocols]).toarray().tolist()[0]
        ...
        if pkt.transport_layer != None:
            if protocol == socket.IPPROTO_TCP:
                ...
                pf.features_list.append(
                    int(pkt.tcp.window_size_value))  # TCP window size
                pf.features_list = pf.features_list + \
                    [0, 0]  # UDP + ICMP positions
            elif protocol == socket.IPPROTO_UDP:
                pf.features_list = pf.features_list + \
                    [0, 0, 0, 0]  # TCP positions
            ...
            pf.features_list = pf.features_list + \
                [0, 0, 0, 0, 0]  # TCP and UDP positions
            pf.features_list.append(int(pkt.icmp.type))  # ICMP type
        else:
            pf.features_list = pf.features_list + \
                [0, 0, 0, 0, 0, 0]  # padding for layer3-only packets
            tmp_id[4] = 0
...
def process_pcap(pcap_file, dataset_type, in_labels, max_flow_len, labelled_flows, max_flows=0, traffic_type='all', time_window=TIME_WINDOW):
    ...
    print('Completed file {} in {} seconds.'.format(
        pcap_name, time.time() - start_time))
    ...
    if isinstance(cap, pyshark.LiveCapture) == True:
        for pkt in cap.sniff_continuously():
            ...
            temp_dict = store_packet(
                pf, temp_dict, start_time_window, max_flow_len)
    ...
    if pf is not None:
        if pf.id_fwd in temp_dict and start_time_window in temp_dict[pf.id_fwd] and \
            ...
            if pf.id_fwd not in temp_dict and pf.id_bwd not in temp_dict:
                temp_dict[pf.id_fwd] = {start_time_window: np.array(
                    [pf.features_list]), 'label': 0}
                ...
                temp_dict[pf.id_bwd][start_time_window] = np.array(
                    [pf.features_list])
    return temp_dict
...
def apply_labels(flows, labelled_flows, labels, traffic_type):
    for five_tuple, flow in flows.items():
        if labels is not None:
            # for IDS2017/IDS2018 dataset the labels have shorter keys
        ...
        # we only want malicious flows from this dataset
        if traffic_type == 'ddos' and flow['label'] == 0:
            continue
        # we only want benign flows from this dataset
...
def balance_dataset(flows, total_fragments=float('inf')):
    ...
    if ddos_fragments == 0 or benign_fragments == 0:
        ...
        min_fragments = min(total_fragments/2,
                            ddos_fragments, benign_fragments)
...
def main(argv):
    ...
    parser.add_argument(
        '--no_split', help='Do not split the dataset', action='store_true')
    ...
    if args.dataset_folder is not None and args.dataset_type is not None:
        ...
        in_labels = parse_labels(
            args.dataset_type[0], args.dataset_folder[0], label=args.label)
        ...
        filename = str(int(time_window)) + 't-' + \
            str(max_flow_len) + 'n-' + dataset_id + '-preprocess'
        ...
        (total_flows, ddos_flows, benign_flows),  (total_fragments,
                                                   ddos_fragments, benign_fragments) = count_flows(preprocessed_flows)
    ...
    if args.preprocess_folder is not None or args.preprocess_file is not None:
        ...
        for file in filelist:
            ...
            current_time_window = int(filename.split(
                '-')[0].strip().replace('t', ''))
            current_max_flow_len = int(
                filename.split('-')[1].strip().replace('n', ''))
        ...
        for file in filelist:
            with open(file, 'rb') as filehandle:
                ...
                preprocessed_flows = preprocessed_flows + \
                    pickle.load(filehandle)

        # balance samples and redux the number of samples when requested
        preprocessed_flows, benign_fragments, ddos_fragments = balance_dataset(
            preprocessed_flows, args.samples)

        if len(preprocessed_flows) == 0:
            print("Empty dataset!")
            exit()

        preprocessed_train, preprocessed_test = train_test_split(
            preprocessed_flows, train_size=TRAIN_SIZE, shuffle=True)
        preprocessed_train, preprocessed_val = train_test_split(
            preprocessed_train, train_size=TRAIN_SIZE, shuffle=True)
        ...
        output_file = output_folder + '/' + \
            str(time_window) + 't-' + str(max_flow_len) + \
            'n-' + dataset_id + '-dataset'
        if args.no_split == True:  # don't split the dataset
            norm_X_full = normalize_and_padding(
                X_full, mins, maxs, max_flow_len)
            ...
            norm_X_train = normalize_and_padding(
                X_train, mins, maxs, max_flow_len)
            norm_X_val = normalize_and_padding(X_val, mins, maxs, max_flow_len)
            norm_X_test = normalize_and_padding(
                X_test, mins, maxs, max_flow_len)
    ...
    if args.balance_folder is not None and args.output_folder is not None:
        ...
        for file in datasets:
            ...
            if 'train' in filename:
                key = filename.split('dataset')[0].strip(
                ) + 'dataset-balanced-train.hdf5'
                ...
                key = filename.split('dataset')[
                    0].strip() + 'dataset-balanced-val.hdf5'
                if output_filename_prefix == None:
                    output_filename_prefix = filename.split('IDS')[0].strip()
                else:
                    if filename.split('IDS')[0].strip() != output_filename_prefix:
        ...
        total_flows = final_y['train'].shape[0] + \
            final_y['val'].shape[0]+final_y['test'].shape[0]
        ddos_flows = np.count_nonzero(
            final_y['train'])+np.count_nonzero(final_y['val'])+np.count_nonzero(final_y['test'])
